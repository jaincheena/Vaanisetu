...
@router.get("")
async def impact_summary(current_user: dict = Depends(get_current_user)):
    return get_impact_summary()


@router.get("/config")
async def impact_config(current_user: dict = Depends(get_current_user)):
    return get_impact_config()


@router.post("/config")
async def update_config(body: ImpactConfigUpdate, current_user: dict = Depends(get_current_user)):

...
backend/models/pool.py and backend/pipeline/queue.py.
...
def test_translate_generate_overlap() -> None:
    ...
    tmap, files = processor._stage_translate_and_generate(
        job_id="test-overlap", segments=SEGMENTS, source_lang="English",
        target_langs=TARGETS, original_path="/fake/input.mp4",
...
def test_job_queue() -> None:
    ...
    import backend.pipeline.processor as processor
    import backend.pipeline.queue as q

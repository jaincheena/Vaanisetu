...
**Note on Security:** VaaniSetu operates with JWT Authentication (HS256, 7-day expiry). Default administrative seating is provided out of the box (Username: `admin`, Password: `baif2026`). In offline mode, there is a graceful fallback to local auth. Non-admin accounts will have read-only or restricted access to specific impact ledgers and translation glossaries.

---

## Supported Languages
VaaniSetu supports **22 official Indian languages** (Hindi, Marathi, Bengali, Telugu, Tamil, Gujarati, Urdu, Kannada, Odia, Malayalam, Punjabi, Assamese, Maithili, Sanskrit, Konkani, Sindhi, Dogri, Kashmiri, Manipuri, Nepali, Bodo, Santhali) + English.
...
| faster-whisper | 1.1+ | Speech-to-text (CTranslate2 INT8 with Silero VAD) | MIT |
| IndicTrans2 dist-200M | 2.0+ | Translation (INT8 dynamic quantization CPU, float16 CUDA) | MIT |
...
| `C:\VaaniSetu\models\piper\` | ~50–150 MB | Piper TTS ONNX voice models (22 Indic languages) |
